...
class Users {
    ...
    function add($f3) {
        $db = $f3->get('DB');
        $user = new DB\SQL\Mapper($db,$f3->get('user_db_name'));
        ...
        if ( $user->count(array('email=?', $posted_email)) > 0){
            ...
            already an account associated with $posted_email. Did you forget your password?"));
            echo Template::instance()->render('main.html');
        }
        else {
    ...
    function reset_stage($f3) {
        ...
        $message = "We have received a request to reset your password on" . $f3->get('app_name') . " To change your password, please visit " . $f3->get('base_url') . "/users/new_password_prompt/" . $key . "\n\n If you did not request this password change, please ignore this email.";
        $mail = $postmark->to($f3->get('POST.forgot_email'))
        ->subject($f3->get('app_name') . ':Reset Your Password')
